...
/** @defgroup UART_Private_Defines
  * @{
  */
#define INPUT_CLOCK_DIV_1_1          ((uint8_t)0x02)
#define INPUT_CLOCK_DIV_1_2          ((uint8_t)0x00)
#define INPUT_CLK_DIV_MASK           ((uint8_t)0x01)
#define BRCKSEL_T0_MASK              ((uint8_t)0x02)
...
#define MOD1_BIT0_CLEAR        ((uint32_t)0x000000FE)
...
/**
  * @brief  select input clock for prescale.
  * @param  UARTx: Select the UART channel.
  *   This parameter can be one of the following values:
  *   UART0, UART1, UART2, UART3.
  * @param  ClkDivider: Select the input clock divider for prescaler.
  *   This parameter can be UART_DIVIDE_1_1 or UART_DIVIDE_1_2.
  * @retval None
  * @register The used register:
  *   SCxEN<BRCKSEL>(x can be 0,1,2,3)
  */
void UART_SetInputClock(TSB_SC_TypeDef * UARTx, uint8_t ClkDivider)
{
    /* Check the parameters */
    uint8_t divider = 0U;
    uint32_t value = UARTx->EN;
    assert_param(IS_UART_PERIPH(UARTx));
    assert_param(IS_UART_INPUTCLK_DIVIDE_LEVEL(ClkDivider));

    /* Set the value of inputclock for prescaler */
    if (ClkDivider == UART_DIVIDE_1_1) {
        /* Set ¦ÕT0 for input clock */
        divider = INPUT_CLOCK_DIV_1_1;
    } else {
        /* Set ¦ÕT0/2 for input clock */
        divider = INPUT_CLOCK_DIV_1_2;
    }
    /* Set SCxEN<BRCKSEL> to select input clock for prescaler */
    value &= INPUT_CLK_DIV_MASK;
    value |= divider;
    UARTx->EN = value;
}
...
/**
  ...
  *   BUSY or DONE.
  * @register The used register:
  *   SCxMOD2<TBEMP><RBFLL> (x can be 0,1,2,3) 
  */
...
/**
  ...
  * @retval None
  * @register The used register:
  *   SCxMOD2<SWRST[1:0]> (x can be 0,1,2,3)
  */
...
/**
  ...
  *   SCxMOD0<SM[1:0]><SC[1:0]><CTSE><RXE>
  *   SCxMOD1<TXE>
  *   SCxMOD2<SBLEN><WBUF>
  *   SCxCR<EVEN><PE>
  *   SCxEN<BRCKSEL><SIOE>
  *   SCxBRCR<BRCK[1:0]><BRS[3:0]>
  *   SCxBRADD<BRK[3:0]>
  *   (x can be 0,1,2,3)
  */
void UART_Init(TSB_SC_TypeDef * UARTx, UART_InitTypeDef * InitStruct)
{
    ...
    /* Get SCxEN<BRCKSEL> to config input clock for prescaler */
    value = ((uint8_t) (UARTx->EN & BRCKSEL_T0_MASK));
    if (value == INPUT_CLOCK_DIV_1_2) {
        /* Do nothing */
    } else {
        T0 = T0 * 2U;
    }
...
/**
  ...
  * @register The used register:
  *   SCxCR<OERR><PERR><FERR> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @retval None
  * @register The used register:
  *   SCxMOD1<FDPX[1:0]> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @retval None
  * @register The used register:
  *   SCxFCNF<RXTXCNT> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @retval None
  * @register The used register:
  *   SCxRFC<RIL[1:0]> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @retval None
  * @register The used register:
  *   SCxTFC<TIL[1:0]> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @register The used register:
  *   SCxRST<RLVL[2:0]> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @register The used register:
  *   SCxTST<TLVL[2:0]> (x can be 0,1,2,3)
  */
...
/**
  ...
  * @note SIO_SetInputClock need to use before SIO_Init.
  * @register The used registers:
  *   SCxCR<IOC><SCLKS><TIDLE><TXDEMP><EHOLD>
  *   SCxMOD0<SM><RXE>
  *   SCxMOD1<SINT><TXE><FDPX>
  *   SCxMOD2<DRCHG><WBUF>
  *   SCxBRCR<5:0>
  *   (x can be 0,1,2,3)
  */
...
#endif                          /* defined(__TMPM475_UART_H)  */
