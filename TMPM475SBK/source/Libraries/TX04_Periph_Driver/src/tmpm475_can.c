...
void CAN_SetMbxDirection(uint32_t MbxNum, uint8_t MbxDirection)
{
    ...
    assert_param(IS_CAN_MBX_NUM(MbxNum));

    /* Mailbox direction, tx: CANMD<MDn>=0, rx: CANMD<MDn>=1 */
    if (MbxDirection == CAN_MBX_TX) {
        assert_param(IS_CAN_GLOBAL_MBX_NUM(MbxNum));    /*Mailbox 31 is the receive-only */
        TSB_CAN->MD &= ~MbxNum;
    } else {
        TSB_CAN->MD |= MbxNum;
    }
